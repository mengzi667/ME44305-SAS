...
vessel_size = 10   # in total 10 vessels per day, simulation for 10 days
...
# )
main_storage_level = 1892  # central storage 18920 m3
...
def calculate_infrastructure_cost(
    ...
    print(
        f"Central Storage Capacity: {main_storage_capacity_m3:.2f} m3 ({central_storage_capacity_gallons:.2f} gallons)"
    )
    ...
    if substorage_capacity_m3 > 0:
        ...
        single_substorage_cost = (
            TANK_COST_SLOPE * substorage_capacity_gallons + TANK_COST_INTERCEPT
        )
        total_substorage_cost = single_substorage_cost * num_stations
        total_tank_cost += total_substorage_cost
        print(
            f"Single Sub-Storage Tank Capacity: {substorage_capacity_m3:.2f} m3 ({substorage_capacity_gallons:.2f} gallons)"
        )
        print(f"Single Sub-Storage Tank Cost: ${single_substorage_cost:.2f}")
        print(
            f"Total Sub-Storage Tank Cost ({num_stations} units): ${total_substorage_cost:.2f}"
        )
    else:
    ...
    print(
        f"Total Station Fixed Construction Cost ({num_stations} stations): ${total_station_fixed_cost:.2f}"
    )

    # 4. Calculate Pipe Costs
    # Pipe 0: Central Storage to Converter
    fc0_tons_per_day = (
        pipe_flow_capacities_m3_per_hr[0] * AMMONIA_DENSITY_TON_PER_M3 * 24
    )
    ...
    fc1_tons_per_day = (
        pipe_flow_capacities_m3_per_hr[1] * AMMONIA_DENSITY_TON_PER_M3 * 24
    )
    ...
    print(
        f"Pipe 1 (Converter-Station) Flow Capacity: {fc1_tons_per_day:.2f} tons/day, Length: {pipe_lengths_km['converter_to_station']} km/station"
    )
    ...
    fc2_tons_per_day = (
        pipe_flow_capacities_m3_per_hr[2] * AMMONIA_DENSITY_TON_PER_M3 * 24
    )
    ...
    print(
        f"Pipe 2 (In-Station to Vessel) Flow Capacity: {fc2_tons_per_day:.2f} tons/day, Length: {pipe_lengths_km['station_to_vessel']} km/station"
    )
    ...
    total_infrastructure_cost = (
        total_tank_cost + total_pipe_cost + total_station_fixed_cost
    )
...
plt.ylabel("Demand [m3]")
plt.grid()
plt.legend([("Vessel " + str(i)) for i in range(vessel_size)])
...
plt.grid()
plt.xlabel("Time [mins]")
plt.ylabel("Substorage level [m3]")
plt.legend([("Station " + str(i)) for i in range(station_num)])
...
plt.xlabel("Time [mins]")
plt.annotate(
    f"Initial Main Storage Level: {main_storage_timeline[0][1]:.2f}",
    (main_storage_timeline[0][0], main_storage_timeline[0][1]),
)
plt.annotate(
    f"Final Main Storage Level: {main_storage_level:.2f}",
    ((main_storage_timeline[-1][0] - 500, main_storage_timeline[-1][1])),
)
plt.ylabel("Central storage level [m3]")
...
P2 = (
    sum(threshold_good < t <= threshold_acceptable for t in vessel_service_time)
    / vessel_size
)
